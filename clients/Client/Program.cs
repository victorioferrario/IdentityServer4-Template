...
namespace Client
{
    class Program
    {
        ...
        static async Task<bool> RunProcess(IdentityClient identity, bool isAdminClient = false)
        {
            ...
            if (await RequestApiAsync(identity))
            {
                if (isAdminClient)
                {
                    await RequestDataSetApiAsync(identity);
                }
            }
            return true;
        }
        static async Task<TokenResponse> RequestTokenAsync(IdentityClient identity)
        {
            ...
            Console.Write(client.ClientId, client.ClientSecret, identity.ClientScope);
        ...
        static async Task<bool> RequestDataSetApiAsync(Hubx.Net.Identity.Models.IdentityClient identity)
        {
            var client = new HttpClient();            
            client.SetBearerToken(identity.TokenResponse.AccessToken);            
        ...
        static async Task<bool> RequestDataSetNotAuthenticatedApiAsync(Hubx.Net.Identity.Models.IdentityClient identity)
        {
            var client = new HttpClient();           
            var response = await client.GetAsync(identity.ResourceUri + "/dataset");
            if (!response.IsSuccessStatusCode)
